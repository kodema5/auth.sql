\if :{?auth_config_config_user_sql}
\else
\set auth_web_config_config_user_sql true

create function auth.config_user (
...
begin
    ...
    then
        with
        deleted as (
            delete from _auth.config_user c
            using (
                select id, name
